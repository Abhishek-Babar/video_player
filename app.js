...
$(document).ready(function() {
    ...
    function playCard(mObj) {
        $("#video-player").attr("src","https://player.vimeo.com/video/" + mObj.vimeoId);
        let views = mObj.views;
        if(String(mObj.views).length > 3) {
           views = String(mObj.views);
           views = views.slice(0 , views.length - 3) + "k";
        } 
        $("#views-count").html(views);
        $("#video-title").html(mObj.title);
        $("#video-description").html(mObj.description);
        $(window).scrollTop(0);
    }
    for(let i = 0; i < playlistData.length; i++){
            const playListCard = createPlaylistCard(playlistData[i], i+1);
            $("#playlist-wrapper").append(playListCard);
    }
    playCard(videoPlaySectionData[0]);
    ...
    $(".playlist-card").click(function() {
        ...
        playCard(videoPlaySectionData[id - 1]);
        $(".playlist-card").removeClass("active-card");
        $("#" + this.id).addClass("active-card");
    })
});